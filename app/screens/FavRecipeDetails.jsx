        ...
        {text: 'OK', onPress: async () => {
          ...
          if(response){
            Alert.alert(
              ...
              '',
              [
                { text : 'OK', onPress : ()=> props.navigation.navigate('Favourites')}
              ],
              { cancelable : false }
            )
          }
        ...
        headerRight: () => (
          <View style={{marginHorizontal : 10}}>
            <TouchableNativeFeedback onPress={()=>handleShare()}>
              <MaterialIcons name="share" style={{ fontSize : 30, color : 'white'}}/>
            </TouchableNativeFeedback>
            <TouchableNativeFeedback onPress={()=>handleDeleteFav()}>
              <MaterialIcons name="delete" style={{ fontSize : 30, color : 'white'}}/>
            </TouchableNativeFeedback>
          </View>
        ),
      });
